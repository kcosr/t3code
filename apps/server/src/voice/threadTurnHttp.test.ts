...
describe("native thread turn HTTP", () => {
  ...
  it.effect("rejects every operation endpoint after child authority is revoked", () =>
    runWithServer(
      (baseUrl) => {
        ...
        const headers = {
          "x-t3-voice-operation": "revoked-operation-token",
          "x-t3-voice-runtime-protocol-major": "1",
        };
        return Effect.all(
          [
  ...
  it.effect("accepts only the strict draft disposition body", () =>
    runWithServer(
      (baseUrl) => {
        ...
        const headers = {
          "x-t3-voice-operation": "operation-token",
          "x-t3-voice-runtime-protocol-major": "1",
        };
        return Effect.all([
          HttpClient.post(url, {
  ...
  it.effect("rejects an incompatible runtime protocol before authorization", () =>
    runWithServer(
      (baseUrl) =>
        HttpClient.get(
          new URL("/api/voice/runtime/thread-turns/native-thread-operation/events", baseUrl),
          { headers: { "x-t3-voice-operation": "operation-token" } },
        ).pipe(Effect.map((response) => response.status)),
      { expectedAuthorizationCalls: 0 },
    ).pipe(Effect.tap((status) => Effect.sync(() => expect(status).toBe(426)))),
  );
});
