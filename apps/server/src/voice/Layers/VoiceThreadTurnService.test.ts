...
import { VoiceThreadTurnServiceLive } from "./VoiceThreadTurnService.ts";
import { __testing } from "./VoiceThreadTurnService.ts";

const authSessionId = AuthSessionId.make("native-thread-service-auth");
const runtimeId = VoiceRuntimeId.make("native-thread-service-runtime");
...
const makeTestLayer = Effect.fn("test.makeNativeThreadTurnServiceLayer")(function* (
  ...
  const store = VoiceThreadTurnStoreLive.pipe(Layer.provide(sqlite));
          ...
          Effect.succeed(
            token === runtimeToken
              ? {
                  ...
                  targetDigest: VoiceRuntimeTargetDigest.make(
  ...
  return {
    layer: VoiceThreadTurnServiceLive.pipe(Layer.provideMerge(dependencies)),
...
describe.sequential("VoiceThreadTurnService", () => {
  it("suppresses segment advertisement for disabled or detached speech", () => {
    expect(__testing.shouldAdvertiseSpeech({ speechEnabled: false, detachedAt: null })).toBe(false);
    expect(
      __testing.shouldAdvertiseSpeech({
        speechEnabled: true,
        detachedAt: "2026-07-14T00:00:00.000Z",
      }),
    ).toBe(false);
    expect(__testing.shouldAdvertiseSpeech({ speechEnabled: true, detachedAt: null })).toBe(true);
  });
  ...
  it.effect("survives parent expiry but denies auth revocation without deleting the receipt", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* sql`UPDATE voice_runtime_grants SET expires_at = -1
          WHERE auth_session_id = ${authSessionId} AND runtime_id = ${runtimeId}`;
  ...
  it.effect("encrypts draft transcripts, reports corruption, and consumes atomically", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* sql`UPDATE voice_thread_turn_drafts
        ...
        expect(
          ...
          SELECT ciphertext FROM voice_thread_turn_drafts
          WHERE operation_id = ${operation.snapshot.operationId}`,
