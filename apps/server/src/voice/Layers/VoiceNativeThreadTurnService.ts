...
const mapPhase = (phase: PersistedVoiceNativeThreadTurn["phase"]): VoiceRuntimeThreadTurnPhase => {
  switch (phase) {
    case "created":
    case "transcribing":
    case "dispatching":
...
const snapshot = (
  record: PersistedVoiceNativeThreadTurn,
  correlation: VoiceNativeThreadTurnReceiptCorrelation,
...
const make = Effect.gen(function* () {
  ...
  const hydrateSnapshot = Effect.fn("VoiceNativeThreadTurnService.snapshot")(function* (
    record: PersistedVoiceNativeThreadTurn,
  ) {
  ...
  const monitorLoop = Effect.fn("VoiceNativeThreadTurnService.monitor")(function* (
    ...
    while (true) {
      ...
      if (operation.detachedAt === null && turn.assistantMessageId !== null) {
  ...
  const reconcileOrDispatch = Effect.fn("VoiceNativeThreadTurnService.reconcileOrDispatch")(
    function* (
      operation: PersistedVoiceNativeThreadTurn,
  ...
  const setDraftDisposition: VoiceNativeThreadTurnService["Service"]["setDraftDisposition"] =
    Effect.fn("VoiceNativeThreadTurnService.setDraftDisposition")(
      function* (operationToken, operationId) {
  ...
  const readDraft: VoiceNativeThreadTurnService["Service"]["readDraft"] = Effect.fn(
    "VoiceNativeThreadTurnService.readDraft",
  )(function* (operationToken, operationId) {
  ...
  const consumeDraft: VoiceNativeThreadTurnService["Service"]["consumeDraft"] = Effect.fn(
    "VoiceNativeThreadTurnService.consumeDraft",
  )(function* (operationToken, operationId) {
    const normalizedOperationId = VoiceThreadTurnOperationId.make(operationId);
    const result = yield* store
      .consumeDraft(
  ...
  const detach: VoiceNativeThreadTurnService["Service"]["detach"] = Effect.fn(
    "VoiceNativeThreadTurnService.detach",
  )(function* (operationToken, operationId) {
    const normalizedOperationId = VoiceThreadTurnOperationId.make(operationId);
    const result = yield* store
      .detach(
