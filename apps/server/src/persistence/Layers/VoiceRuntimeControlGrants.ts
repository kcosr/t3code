import { AuthSessionId, VoiceRuntimeId, VoiceSessionId } from "@t3tools/contracts";
...
import {
  ...
  type VoiceRuntimeControlGrantRepositoryShape,
} from "../Services/VoiceRuntimeControlGrants.ts";

const make = Effect.gen(function* () {
  const sql = yield* SqlClient.SqlClient;
  const insert: VoiceRuntimeControlGrantRepositoryShape["insert"] = (grant, now) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          yield* sql`
          INSERT INTO voice_runtime_control_grants (
          ...
          WHERE ${grant.runtimeId ?? null} IS NULL OR EXISTS (
            SELECT 1 FROM voice_runtime_grants
      ...
      .pipe(Effect.mapError(toPersistenceSqlError("VoiceRuntimeControlGrantRepository.insert")));
  const findActive: VoiceRuntimeControlGrantRepositoryShape["findActive"] = (tokenHash, now) =>
    Effect.gen(function* () {
      yield* sql`DELETE FROM voice_runtime_control_grants WHERE expires_at <= ${now}`;
      ...
      const rows = yield* sql<{
        ...
        FROM voice_runtime_control_grants AS child
        LEFT JOIN voice_runtime_grants AS runtime
      ...
      return {
        ...
        capabilities,
        ...(row.runtimeId === null ? {} : { runtimeId: VoiceRuntimeId.make(row.runtimeId) }),
        ...(row.runtimeGeneration === null ? {} : { runtimeGeneration: row.runtimeGeneration }),
      };
    }).pipe(
      Effect.mapError(toPersistenceSqlError("VoiceRuntimeControlGrantRepository.findActive")),
    );
  const releaseSessionControl: VoiceRuntimeControlGrantRepositoryShape["releaseSessionControl"] = (
    sessionId,
  ) =>
    sql`DELETE FROM voice_runtime_control_grants
          WHERE session_id = ${sessionId} AND handoff_actions = 0`.pipe(
      Effect.andThen(
        sql`UPDATE voice_runtime_control_grants SET session_control = 0
              WHERE session_id = ${sessionId} AND handoff_actions = 1`,
      ),
      Effect.asVoid,
      Effect.mapError(
        toPersistenceSqlError("VoiceRuntimeControlGrantRepository.releaseSessionControl"),
      ),
    );
  const completeHandoff: VoiceRuntimeControlGrantRepositoryShape["completeHandoff"] = (sessionId) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          ...
          yield* sql`UPDATE voice_runtime_control_grants
      ...
      .pipe(
        Effect.asVoid,
        Effect.mapError(
          toPersistenceSqlError("VoiceRuntimeControlGrantRepository.completeHandoff"),
        ),
      );
  const revokeSession = (sessionId: VoiceSessionId) =>
    sql`DELETE FROM voice_runtime_control_grants WHERE session_id = ${sessionId}`.pipe(
      Effect.asVoid,
      Effect.mapError(toPersistenceSqlError("VoiceRuntimeControlGrantRepository.revokeSession")),
    );
  const revokeAuthSession = (authSessionId: AuthSessionId) =>
    sql`DELETE FROM voice_runtime_control_grants WHERE auth_session_id = ${authSessionId}`.pipe(
      Effect.asVoid,
      Effect.mapError(
        toPersistenceSqlError("VoiceRuntimeControlGrantRepository.revokeAuthSession"),
      ),
    );
  const revokeRuntime = (authSessionId: AuthSessionId, runtimeId: VoiceRuntimeId) =>
    sql`DELETE FROM voice_runtime_control_grants WHERE auth_session_id = ${authSessionId}
      AND runtime_id = ${runtimeId}`.pipe(
      Effect.asVoid,
      Effect.mapError(toPersistenceSqlError("VoiceRuntimeControlGrantRepository.revokeRuntime")),
    );
  return VoiceRuntimeControlGrantRepository.of({
    insert,
    findActive,
...
export const VoiceRuntimeControlGrantRepositoryLive = Layer.effect(
  VoiceRuntimeControlGrantRepository,
  make,
);
