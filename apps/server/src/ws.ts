...
import * as ClientCommandDispatcher from "./orchestration/Services/ClientCommandDispatcher.ts";
...
const makeWsRpcLayer = (
  ...
  clientCommandDispatcher: ClientCommandDispatcher.ClientCommandDispatcher["Service"],
) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      return WsRpcGroup.of({
        [ORCHESTRATION_WS_METHODS.dispatchCommand]: (command) =>
          observeRpcEffect(
            ...
            clientCommandDispatcher.dispatch(command).pipe(
              Effect.mapError((cause) =>
                isOrchestrationDispatchCommandError(cause)
                  ? cause
                  : cause._tag === "ClientCommandNormalizationError" &&
                      isOrchestrationDispatchCommandError(cause.cause)
                    ? cause.cause
...
export const websocketRpcRouteLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const clientCommandDispatcher = yield* ClientCommandDispatcher.ClientCommandDispatcher;
    return HttpRouter.add(
      ...
      Effect.gen(function* () {
        ...
        const rpcWebSocketHttpEffect = yield* RpcServer.toHttpEffectWebsocket(WsRpcGroup, {
          ...
          Effect.provide(
            makeWsRpcLayer(session, previewAutomationBroker, clientCommandDispatcher).pipe(
