...
internal class T3VoiceBackgroundSnapshotStore(
  ...
  @Synchronized
  fun write(snapshot: T3VoiceBackgroundSnapshot) {
    check(
      storage.put(
        mapOf(
    ...
    ) { "Could not persist the background voice snapshot." }
  }

  @Synchronized
  fun read(): T3VoiceBackgroundSnapshot {
    val phase = storage.getString(KEY_PHASE) ?: return T3VoiceBackgroundSnapshot()
    return try {
      T3VoiceBackgroundSnapshot(
        runtimeId = storage.getString(KEY_RUNTIME_ID),
        readinessGeneration = required(KEY_GENERATION).toLong(),
        mode = storage.getString(KEY_MODE)?.let(T3VoiceBackgroundMode::valueOf),
        phase = T3VoiceBackgroundPhase.valueOf(phase),
        ...
        terminalSummary =
          storage.getString(KEY_TERMINAL_SUMMARY)?.let(T3VoiceBackgroundTerminalSummary::valueOf),
      )
    } catch (_: Exception) {
      clear()
      T3VoiceBackgroundSnapshot()
  ...
  @Synchronized
  fun clear() {
    check(storage.clear(ALL_KEYS)) { "Could not clear the background voice snapshot." }
  }

  private fun required(key: String): String =
    checkNotNull(storage.getString(key)) { "Missing background voice snapshot field." }

  private fun requiredBoolean(key: String): Boolean =
    when (val value = required(key)) {
      "true" -> true
      "false" -> false
      else -> error("Invalid background voice snapshot boolean: $value")
    }
