...
internal object VoiceRuntimeOriginPolicy {
  fun normalize(origin: String): String {
    val uri = URI(origin)
    require(uri.scheme.equals("https", ignoreCase = true)) {
      "Runtime voice execution requires HTTPS."
    }
    require(!uri.host.isNullOrBlank() && uri.userInfo === null) {
      "Invalid runtime voice origin."
    }
    require(uri.rawQuery === null && uri.rawFragment === null) {
      "Runtime voice origins cannot contain a query or fragment."
    }
    require(uri.rawPath.isNullOrEmpty() || uri.rawPath == "/") {
      "Runtime voice origins cannot contain a path."
    }
    require(uri.port in -1..65535) { "Invalid runtime voice origin port." }
...
internal enum class VoiceRuntimeHttpMethod {
  GET,
  POST,
  PUT,
}
...
internal interface VoiceRuntimeRequestBody {
...
internal data class VoiceRuntimeByteArrayBody(
...
internal data class VoiceRuntimeHttpRequest(
  val origin: String,
  val path: String,
  ...
  init {
    VoiceRuntimeOriginPolicy.endpoint(origin, path, queryParameters)
    require(maximumRequestBytes in 0..ABSOLUTE_MAXIMUM_REQUEST_BYTES) {
      "Invalid runtime request limit."
    }
    require(maximumResponseBytes in 1..ABSOLUTE_MAXIMUM_RESPONSE_BYTES) {
      "Invalid runtime response limit."
    }
    require((method != VoiceRuntimeHttpMethod.GET) || body === null) {
      "GET runtime requests cannot contain a body."
    }
    require(body === null || body.contentLength in 0..maximumRequestBytes) {
      "Runtime request body exceeds its limit."
    }
    require(body === null || validContentType(body.contentType)) {
      "Invalid runtime request content type."
    }
  }
...
internal enum class VoiceRuntimeHttpFailureKind {
  AUTHORITY_REJECTED,
  CONFLICT,
  RETRYABLE,
  PERMANENT,
...
internal sealed interface VoiceRuntimeHttpResult {
  data class Success(
    val statusCode: Int,
    val contentType: String?,
    val body: ByteArray,
    val headers: Map<String, String> = emptyMap(),
  ) : VoiceRuntimeHttpResult
...
internal object VoiceRuntimeHttpPolicy {
  fun classify(statusCode: Int): VoiceRuntimeHttpFailureKind? =
    when (statusCode) {
      in 200..299 -> null
      HttpURLConnection.HTTP_UNAUTHORIZED,
      HttpURLConnection.HTTP_FORBIDDEN,
      -> VoiceRuntimeHttpFailureKind.AUTHORITY_REJECTED
      HttpURLConnection.HTTP_CONFLICT -> VoiceRuntimeHttpFailureKind.CONFLICT
      HttpURLConnection.HTTP_CLIENT_TIMEOUT,
      425,
      429,
      in 500..599,
      -> VoiceRuntimeHttpFailureKind.RETRYABLE
      else -> VoiceRuntimeHttpFailureKind.PERMANENT
    }
}
...
internal class T3VoiceBoundedInputStream(
  ...
  override fun read(): Int {
    ...
    check(consumed < maximumBytes || value == -1) { "Runtime response exceeded its limit." }
  ...
  override fun read(buffer: ByteArray, offset: Int, length: Int): Int {
    if (consumed == maximumBytes) {
      return if (super.read() == -1) -1 else error("Runtime response exceeded its limit.")
    }
...
internal class VoiceRuntimeHttpTransport(
  private val openConnection: (URL) -> HttpsURLConnection = {
    it.openConnection() as HttpsURLConnection
  },
) {
  fun newCall(request: VoiceRuntimeHttpRequest): VoiceRuntimeHttpCall =
    VoiceRuntimeHttpCall(request, openConnection)

  fun execute(request: VoiceRuntimeHttpRequest): VoiceRuntimeHttpResult =
    newCall(request).execute()
}

internal class VoiceRuntimeHttpCall(
  private val request: VoiceRuntimeHttpRequest,
  private val openConnection: (URL) -> HttpsURLConnection,
) {
  ...
  fun execute(): VoiceRuntimeHttpResult {
    check(executed.compareAndSet(false, true)) { "A runtime HTTP call can execute only once." }
    ...
    return try {
      ...
      val failure = VoiceRuntimeHttpPolicy.classify(statusCode)
      if (failure != null) {
        discardErrorBody(connection.errorStream)
        if (cancelled.get()) cancelledResult() else VoiceRuntimeHttpResult.Failure(failure, statusCode)
      } else {
        val body =
  ...
  fun executeStreaming(onChunk: (ByteArray) -> Unit): VoiceRuntimeHttpResult {
    ...
    if (cancelled.get()) return cancelledResult()
    val connection = try {
      openConnection(VoiceRuntimeOriginPolicy.endpoint(
        request.origin, request.path, request.queryParameters,
      ))
    } catch (_: Exception) {
      return if (cancelled.get()) cancelledResult() else retryableResult()
    }
    ...
    return try {
      ...
      if (failure != null) {
        discardErrorBody(connection.errorStream)
        if (cancelled.get()) cancelledResult()
        else VoiceRuntimeHttpResult.Failure(failure, statusCode)
      } else {
        if (request.path.contains("/thread-turns/") && request.path.contains("/speech/")) {
          check(connection.contentType?.substringBefore(';')?.trim() == "audio/pcm")
          check(
            connection.getHeaderField("x-t3-audio-format") ==
              "s16le;rate=24000;channels=1",
          )
        }
        connection.inputStream.use { input ->
          T3VoiceBoundedInputStream(input, request.maximumResponseBytes).use { bounded ->
            val buffer = ByteArray(STREAM_CHUNK_BYTES)
            var carry: Byte? = null
            while (true) {
              val offset = if (carry == null) 0 else 1.also { buffer[0] = carry!! }
              val read = bounded.read(buffer, offset, buffer.size - offset)
              if (read == -1) {
                check(carry == null) { "PCM response ended on a partial frame." }
                break
              }
              if (cancelled.get()) return cancelledResult()
              val available = offset + read
              val emitted = available - (available % Short.SIZE_BYTES)
              if (emitted > 0) onChunk(buffer.copyOf(emitted))
              carry = if (emitted < available) buffer[available - 1] else null
            }
          }
        }
        VoiceRuntimeHttpResult.Success(
  ...
  private fun responseHeaders(connection: HttpsURLConnection): Map<String, String> =
    listOf("x-t3-audio-format").mapNotNull { name ->
      connection.getHeaderField(name)?.takeIf {
        it.length <= 128 && '\r' !in it && '\n' !in it
      }?.let { name to it }
  ...
  private fun copyBounded(input: InputStream, output: java.io.OutputStream, maximumBytes: Long) {
    ...
    while (true) {
      ...
      if (read == -1) break
      if (cancelled.get()) throw VoiceRuntimeHttpCancelledException
      total = Math.addExact(total, read.toLong())
      check(total <= maximumBytes) { "Runtime request body exceeds its limit." }
  ...
  private fun cancelledResult() =
    VoiceRuntimeHttpResult.Failure(VoiceRuntimeHttpFailureKind.CANCELLED, null)

  private fun permanentResult() =
    VoiceRuntimeHttpResult.Failure(VoiceRuntimeHttpFailureKind.PERMANENT, null)

  private fun retryableResult() =
    VoiceRuntimeHttpResult.Failure(VoiceRuntimeHttpFailureKind.RETRYABLE, null)

  private companion object {
    const val CONNECT_TIMEOUT_MILLIS = 5_000
    const val READ_TIMEOUT_MILLIS = 30_000
    const val MAXIMUM_ERROR_BYTES = 4_096
    const val STREAM_CHUNK_BYTES = 64 * 1_024
  }
}

private data object VoiceRuntimeHttpCancelledException : IllegalStateException()
