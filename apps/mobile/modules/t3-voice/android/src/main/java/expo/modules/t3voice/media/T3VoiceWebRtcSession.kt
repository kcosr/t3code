...
import expo.modules.t3voice.bridge.T3VoiceSessionIdAssertion
...
internal class T3VoiceWebRtcSession(
  ...
  private data class ActiveSession(
    ...
    var audioRouterActive: Boolean = false,
  ...
  private val sessionIds = T3VoiceSessionIdAssertion(SESSION_ID_TOMBSTONE_CAPACITY)
    ...
    try {
      audioDeviceModule =
        JavaAudioDeviceModule.builder(applicationContext)
          ...
          .setAudioRecordErrorCallback(AudioRecordErrorCallback(sessionId))
          .setAudioTrackErrorCallback(AudioTrackErrorCallback(sessionId))
  ...
  fun prepare(
    ...
    synchronized(lock) {
      ...
      sessionIds.assertFresh(sessionId)
    }
    val prepared = createPeerResources(sessionId)
  ...
  fun drainPlayout(
    ...
    val (session, connectionState, captureState) =
      synchronized(lock) {
        val current = requireActive(sessionId)
        check(current.playoutDrain == null) { "Realtime playout is already draining." }
        current.captureState = T3VoiceCapturePolicy.setInputReady(current.captureState, false)
        applyCaptureState(current)
        val drain =
          PlayoutDrain(
            policy = T3VoiceRealtimePlayoutDrainPolicy(System.nanoTime() / 1_000_000),
        ...
        T3VoiceDiagnostics.record(
          current.diagnosticGeneration,
          T3VoiceDiagnosticCategory.LIFECYCLE,
          T3VoiceDiagnosticCode.HANDOFF_DRAIN_STARTED,
        )
        Triple(current, connectionStateFor(current), current.captureState)
      }
    onStateChanged(
      sessionId,
      connectionState,
      captureState.userMuted,
      captureState.inputReady,
    )
    samplePlayoutDrain(session)
  }

  fun cancelPlayoutDrain(sessionId: String) {
    val drain =
      synchronized(lock) {
        active?.takeIf { it.sessionId == sessionId }?.let { session ->
    ...
    if (drain != null) {
      ...
      T3VoiceDiagnostics.record(
        synchronized(lock) {
          active?.takeIf { it.sessionId == sessionId }?.diagnosticGeneration ?: 0
        },
        T3VoiceDiagnosticCategory.LIFECYCLE,
        T3VoiceDiagnosticCode.HANDOFF_DRAIN_INTERRUPTED,
      )
    }
  }
  ...
  fun stop(sessionId: String): Boolean {
    ...
    if (session.audioRouterActive) audioRouter.stop()
    T3VoiceDiagnostics.record(
      session.diagnosticGeneration,
      ...
      val routeSetup =
        runCatching {
          if (synchronized(lock) { active === session && !session.audioRouterActive }) {
            ...
            var adopted = false
            try {
              runCatching { audioRouter.select(session.audioRouteId) }
                .onFailure {
                  audioRouter.select(T3VoiceAudioRouteKind.SYSTEM.id)
                }
              adopted =
                synchronized(lock) {
                  if (active === session) {
                    session.audioRouterActive = true
                    true
                  } else {
                    false
                  }
  ...
  private fun failOnConnectionTimeout(
    ...
    val session =
      synchronized(lock) {
        val current = active?.takeIf { it === session } ?: return
        val peerState = current.peerConnection.connectionState()
        val shouldFail = peerState != PeerConnection.PeerConnectionState.CONNECTED
        if (!shouldFail) {
  ...
  private fun samplePlayoutDrain(session: ActiveSession) {
    val shouldSample = synchronized(lock) { active === session && session.playoutDrain != null }
    if (!shouldSample) return
    observePlayoutDrain(session)
  }

  private fun observePlayoutDrain(session: ActiveSession) {
    val nowMillis = System.nanoTime() / 1_000_000
    val decision =
      synchronized(lock) {
        if (active !== session) return
        val drain = session.playoutDrain ?: return
        drain.policy.observe(nowMillis, session.playoutMonitor.lastAudibleAtMillis())
      }
    when (decision) {
  ...
  private fun completePlayoutDrain(
    ...
    T3VoiceDiagnostics.record(
      ...
      T3VoiceDiagnosticCategory.LIFECYCLE,
      when (outcome) {
        T3VoiceRealtimePlayoutDrainOutcome.DRAINED -> T3VoiceDiagnosticCode.HANDOFF_DRAINED
        T3VoiceRealtimePlayoutDrainOutcome.TIMED_OUT ->
          T3VoiceDiagnosticCode.HANDOFF_DRAIN_TIMED_OUT
        T3VoiceRealtimePlayoutDrainOutcome.SESSION_ENDED ->
          T3VoiceDiagnosticCode.HANDOFF_DRAIN_INTERRUPTED
      },
    )
  ...
  private inner class AudioRecordErrorCallback(
    ...
    override fun onWebRtcAudioRecordInitError(message: String?) =
      reportAudioError(sessionId, "realtime-microphone-init", message)

    override fun onWebRtcAudioRecordStartError(
      code: JavaAudioDeviceModule.AudioRecordStartErrorCode?,
      message: String?,
    ) = reportAudioError(sessionId, "realtime-microphone-start", message)

    override fun onWebRtcAudioRecordError(message: String?) =
      reportAudioError(sessionId, "realtime-microphone", message)
  }
  ...
  private inner class AudioTrackErrorCallback(
    ...
    override fun onWebRtcAudioTrackInitError(message: String?) =
      reportAudioError(sessionId, "realtime-playout-init", message)

    override fun onWebRtcAudioTrackStartError(
      code: JavaAudioDeviceModule.AudioTrackStartErrorCode?,
      message: String?,
    ) = reportAudioError(sessionId, "realtime-playout-start", message)

    override fun onWebRtcAudioTrackError(message: String?) =
      reportAudioError(sessionId, "realtime-playout", message)
  }
  ...
  private class PlaybackSamplesReadyCallback(
    ...
    override fun onWebRtcAudioTrackSamplesReady(samples: JavaAudioDeviceModule.AudioSamples) {
      ...
      monitor.observePcm16LittleEndian(samples.data, System.nanoTime() / 1_000_000)
    }
  }
  ...
  private fun reportAudioError(
    ...
    fail(sessionId, code, "Realtime audio failed.", null, true)
  }
